...
public class LVolunteerLoginServlet extends HttpServlet{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)  
		...
		if(user2!=null){  
			...
			if(!userType.isEmpty() && "Lead Volunteer".equals(userType)) {
			...
			RequestDispatcher rd=request.getRequestDispatcher("LVolunteerHome.jsp");  
			rd.forward(request,response);  
			}else if(!userType.isEmpty() && !("admin".equals(userType))) {
				request.setAttribute("errorMsg", "Invalid username or password!");  
				RequestDispatcher rd=request.getRequestDispatcher("LVolunteerLogin.jsp");  
				rd.forward(request,response);  
				}
		}  
		else{  
