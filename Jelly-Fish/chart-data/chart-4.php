...
$sql = "SELECT b.species_name as species_name,a.type_of_occurence as type_of_occurence,count(a.type_of_occurence) as frequency from jf_info a,jf_species b,jf_species_data c where a.sno=c.sno and b.species_id=c.species_id and a.sno=c.sno group by b.species_name,a.type_of_occurence ;";
...
while($row = $result->fetch_assoc()) {
    ...
    if (!isset($state_occurrences[$species_name])) {
        $state_occurrences[$species_name] = array_fill(0, count($types_of_occurence), 0);
        $state_totals[$species_name] = 0; // To track total occurrences for sorting
    }

    // Set frequency for the corresponding type of occurrence
    $index = array_search($type_of_occurence, $types_of_occurence);
    if ($index !== false) {
        $state_occurrences[$species_name][$index] = $frequency;
        $state_totals[$species_name] += $frequency; // Add to total for sorting
    }
}
