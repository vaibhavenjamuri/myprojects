...
$sql_range = "
    SELECT 
        b.species_name,
        MIN(CASE WHEN YEAR(a.date_from) BETWEEN 1980 AND 2015 THEN a.latitude END) AS min_lat_1980_2015,
        MAX(CASE WHEN YEAR(a.date_from) BETWEEN 1980 AND 2015 THEN a.latitude END) AS max_lat_1980_2015,
        MIN(CASE WHEN YEAR(a.date_from) > 2015 THEN a.latitude END) AS min_lat_2015_2025,
        MAX(CASE WHEN YEAR(a.date_from) > 2015 THEN a.latitude END) AS max_lat_2015_2025
    FROM jf_info a
...
$sql_bubbles = "
    ...
    JOIN jf_species_data c ON a.sno = c.sno
    JOIN jf_species b ON b.species_id = c.species_id
    WHERE a.latitude IS NOT NULL AND a.date_from IS NOT NULL
";
...
while ($row = $result_bubbles->fetch_assoc()) {
    ...
    $bubbles[] = [
        "x" => (float)$row['latitude'],  // x is now latitude
        "y" => $species_y_map[$species],
        "z" => (float)$row['latitude'],  // colorKey
        "year" => $row['year'],
        "species" => $species
    ];
}
