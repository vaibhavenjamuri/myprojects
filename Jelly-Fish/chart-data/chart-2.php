...
while($row = $result->fetch_assoc()) {
    ...
    if (!isset($state_occurrences[$state])) {
        $state_occurrences[$state] = array_fill(0, count($types_of_occurence), 0);
        $state_totals[$state] = 0; // To track total occurrences for sorting
    }

    // Set frequency for the corresponding type of occurrence
    $index = array_search($type_of_occurence, $types_of_occurence);
    if ($index !== false) {
        $state_occurrences[$state][$index] = $frequency;
        $state_totals[$state] += $frequency; // Add to total for sorting
    }
}
