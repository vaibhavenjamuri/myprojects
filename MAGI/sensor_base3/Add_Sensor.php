...
document.getElementById("hide1").value = total_text;
  ...
  "<div id='input_text"+total_text+"_wrapper' class='row'><div><div class='col-md-3'><input type='text' class='input_text' placeholder='Parameter Name' id='pname"+total_text+"' name='pname"+total_text+"'></div></div><div><div class='col-md-2'><input type='text'  placeholder='Unit' id='unit"+total_text+"' name='unit"+total_text+"' ></div><div class='col-md-2'><input type='text'  placeholder='Minimum Range' id='min"+total_text+"' name='min"+total_text+"' ></div><div class='col-md-2'><input type='text'  placeholder='Maximum Range' id='max"+total_text+"' name='max"+total_text+"'></div></div><div class='col-md-2'><input type='button' value='Remove' class='btn btn-primary' onclick=remove_field('input_text"+total_text+"');></div></div></br>";
...
<body>
    ...
    <div class="container">
            ...
            <div id="main" style="border:solid 1px">
                    ...
                    <div class="row form-group">
					  ...
					  <div class="row form-group">
                        ...
                        <div class="col-md-9">
                            ...
                            <input type="text" class="form-control" name="smake" required>
                        </div>
                    </div>
					 <div class="row form-group">
                        <div class="col-md-3">
                        ...
                        <div class="col-md-9">
                               ...
                               <input type="text" class="form-control " name="smodel" required/>
					 ...
					 <div class="row form-group">
                        ...
                        <div class="col-md-9">
                               ...
                               <input type="text" class="form-control " name="stype" required/>
				...
				<input type="button" value="Add Parameters" class="btn btn-primary" onclick="add_field();">
                <div class="row form-group">
...
$phpVar=$_POST['hide1'];
        ...
        $sql = "SELECT sensor_id from sensor_model_details where sensor_id= ?";
      
			
			     $stmt1 = $mysqli->prepare($sql);
                $stmt1->bind_param("s", $sid);
				  // echo $ddate,$sname,$smodel,$tstation;
				    $c1++;
					// $stmt->execute();
					    ...
					    if ($stmt1->num_rows >0) {
						   // if ($c1 >1){
							// echo "<p><b>Error:</b> Data already exists!!!</p>";
							 echo '<script type="text/javascript">alert("'.$sid.' Data in sensor_model_details already exists");</script>';
						}
									else {
				 echo '<script type="text/javascript">alert("'.$sid.' Inserted");</script>';
                $stmt1->close();
			  
			  
					 $smodel = $_POST['smodel'];
                # insert data into mysql database
                $sql = "INSERT  INTO `sensor_model_details` (`sensor_id`,`sensor_make`,`sensor_model`,`sensor_type`) VALUES (?,?,?,?)";
                $stmt2 = $mysqli->prepare($sql);
				// echo $ddate,$rdate,$sname,$smodel;
                $stmt2->bind_param("ssss", $sid,$smake,$smodel,$stype);
			...
			// echo '<script type="text/javascript">alert("Inserted");</script>';
                    ...
                    $sql = "SELECT id from `Sensor_parameter_Details` WHERE `id` = ? ";
				...
				// }
                    if ($stmt3->execute()) {
                        $stmt3->store_result();
                        if ($stmt3->num_rows >0) {
							 echo '<script type="text/javascript">alert("sid already exists in parameter table");</script>';
                            ...
                            // for ($i=1;$i<=$phpVar;$i++) {
								...
								$min=$_POST['min'.$i];
								$max=$_POST['max'.$i];
								echo $smodel,$stype,$pname,$min,$max;
								
								
                               $sql = "INSERT INTO sensor_parameter_details
(`id`,
`parameter`,
`unit`,
`min_range`,
...
(?,?,?,?,?);";
                ...
                $stmt4->bind_param("sssss", $sid,$pname,$unit,$min,$max);
				// echo '<script type="text/javascript">alert("Inserted Sensor Model Data");</script>';
				$stmt4->execute();
							// }
					if ($stmt4->num_rows >0) {
						// echo "<p><b>Error:</b> Sensor Model Data Inserted Successfully!</p>";
